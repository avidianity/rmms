...
class File extends Model
{
    ...
    public static function process($file)
    {
        if ($file instanceof UploadedFile) {
            return static::processFile($file);
        } elseif (is_string($file)) {
            return static::processURL($file);
        } else {
            throw new InvalidArgumentException(
                sprintf(
                    '%s::%s($url) must be either a string url, base64 encoded file or an instance of Illuminate\Http\UploadedFile. %s given.',
                    static::class,
                    __FUNCTION__,
                    gettype($file)
                )
     ...
     * @return static
     */
    public static function processURL(string $url)
    {
        $data = [];
        ...
        if ($binary === false) {
            if (base64_encode(base64_decode($url, true)) !== $url) {
                throw new InvalidArgumentException(
                    sprintf(
                        '%s::%s($url) must be either a string url, base64 encoded file or an instance of Illuminate\Http\UploadedFile. %s -> \'%s\' given.',
                        static::class,
                        __FUNCTION__,
                        gettype($url),
                        (string)$url,
    ...
    {
        return static::processURL($url, $user);
    }

    public static function generateFileName(string $mimeType)
    {
        return Str::random(40) . '.' . static::convertMimeToExtension($mimeType);
    }
